...
*  get type for name
  ...
  FIELD-SYMBOLS:<fs> TYPE bapi_te_mereqitem.
  CHECK lr_struct IS BOUND.
  LOOP AT lr_struct->components ASSIGNING <comp>.
    CHECK <comp>-type_kind EQ lr_struct->typekind_packed.
*  perform conversion...
*  needes to be implemented
    lf_done = cl_mmpur_constants=>yes.
  ENDLOOP.
*  MOVE-CORRESPONDING IM_CONTAINER TO CH_STRUC.
*  conversion has been done successful -> prevent system to do any
*  conversion
  IF im_name EQ 'CI_EBANDB'.
    ASSIGN ch_struc TO <fs>.
    <fs>-preq_item = im_container+0(5).
    <fs>-zbrand = im_container+5(5).
    <fs>-zcontract = im_container+10(1).
    <fs>-zbudget = im_container+11(10).
    <fs>-zreason = im_container+21(80).
    <fs>-zshychx = im_container+101(40).
    <fs>-zshyshb = im_container+141(40).
    TRY.
        <fs>-zlowestpr = im_container+181(11).
      CATCH cx_sy_conversion_error.
    ENDTRY.
    <fs>-zlowestdt = im_container+192(8).
    TRY.
        <fs>-zlastpr = im_container+200(11).
      CATCH cx_sy_conversion_error.
    ENDTRY.
    <fs>-zlastdt = im_container+211(8).
  ENDIF.
